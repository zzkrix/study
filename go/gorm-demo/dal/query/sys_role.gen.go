...
func newSysRole(db *gorm.DB, opts ...gen.DOOption) sysRole {
	...
	_sysRole.Rid = field.NewString(tableName, "rid")
	_sysRole.RoleName = field.NewString(tableName, "role_name")
	_sysRole.Users = sysRoleManyToManyUsers{
		db: db.Session(&gorm.Session{}),

		RelationField: field.NewRelation("Users", "model.SysUser"),
		Roles: struct {
			field.RelationField
		}{
			RelationField: field.NewRelation("Users.Roles", "model.SysRole"),
		},
	}
...
func (s *sysRole) updateTableName(table string) *sysRole {
	...
	s.Rid = field.NewString(table, "rid")
...
func (s sysRole) clone(db *gorm.DB) sysRole {
	...
	s.Users.db = db.Session(&gorm.Session{Initialized: true})
	s.Users.db.Statement.ConnPool = db.Statement.ConnPool
	return s
}

func (s sysRole) replaceDB(db *gorm.DB) sysRole {
	s.sysRoleDo.ReplaceDB(db)
	s.Users.db = db.Session(&gorm.Session{})
	return s
}
...
func (a sysRoleManyToManyUsers) Model(m *model.SysRole) *sysRoleManyToManyUsersTx {
	return &sysRoleManyToManyUsersTx{a.db.Model(m).Association(a.Name())}
}

func (a sysRoleManyToManyUsers) Unscoped() *sysRoleManyToManyUsers {
	a.db = a.db.Unscoped()
	return &a
}
...
func (a sysRoleManyToManyUsersTx) Find() (result []*model.SysUser, err error) {
	return result, a.tx.Find(&result)
}

func (a sysRoleManyToManyUsersTx) Append(values ...*model.SysUser) (err error) {
...
func (a sysRoleManyToManyUsersTx) Replace(values ...*model.SysUser) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a sysRoleManyToManyUsersTx) Delete(values ...*model.SysUser) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a sysRoleManyToManyUsersTx) Unscoped() *sysRoleManyToManyUsersTx {
	a.tx = a.tx.Unscoped()
	return &a
}
