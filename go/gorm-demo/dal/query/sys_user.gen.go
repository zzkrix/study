...
func newSysUser(db *gorm.DB, opts ...gen.DOOption) sysUser {
	...
	_sysUser.Roles = sysUserManyToManyRoles{
		...
		RelationField: field.NewRelation("Roles", "model.SysRole"),
	}
...
func (s sysUser) clone(db *gorm.DB) sysUser {
	...
	s.Roles.db = db.Session(&gorm.Session{Initialized: true})
	s.Roles.db.Statement.ConnPool = db.Statement.ConnPool
	return s
}

func (s sysUser) replaceDB(db *gorm.DB) sysUser {
	s.sysUserDo.ReplaceDB(db)
	s.Roles.db = db.Session(&gorm.Session{})
	return s
}
...
func (a sysUserManyToManyRoles) Model(m *model.SysUser) *sysUserManyToManyRolesTx {
	return &sysUserManyToManyRolesTx{a.db.Model(m).Association(a.Name())}
}

func (a sysUserManyToManyRoles) Unscoped() *sysUserManyToManyRoles {
	a.db = a.db.Unscoped()
	return &a
}
...
func (a sysUserManyToManyRolesTx) Find() (result []*model.SysRole, err error) {
	return result, a.tx.Find(&result)
}

func (a sysUserManyToManyRolesTx) Append(values ...*model.SysRole) (err error) {
...
func (a sysUserManyToManyRolesTx) Replace(values ...*model.SysRole) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a sysUserManyToManyRolesTx) Delete(values ...*model.SysRole) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a sysUserManyToManyRolesTx) Unscoped() *sysUserManyToManyRolesTx {
	a.tx = a.tx.Unscoped()
	return &a
}
